...
export const products = {
  ...
  createProduct: handleErrorAsync(
    async (req: any, res: Response, next: NextFunction) => {
      ...
      const productNo = await autoIncrementNumber(ProductManagementModel, 'productNo')
      const productManagementObj = await ProductManagementModel.findOne({ productNo })
  ...
  getProductType: handleErrorAsync(
    async (req: any, res: Response, next: NextFunction) => {
      const productTypes = await ProductTypeModel.find({ isDeleted: false }).sort({ productsType: 1 });
  ...
  createProductType: handleErrorAsync(
    async (req: any, res: Response, next: NextFunction) => {
      ...
      const productsTypeIndex = await autoIncrementNumber(ProductTypeModel, 'productsType')
